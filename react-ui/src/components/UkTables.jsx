...
export default function UKTables({ dbname="uk.db", table="node",editMode }) {
...
console.log(data)
  ...
  const handleUpdate = async (row) => {
    ...
    if(!debug.includes(pk))setDebug([...debug,pk]);
  };
  ...
  const handleDelete = async (row) => {
    ...
    const updated = rows.filter((_, i) => i !== pk);
...
*/
      ...
      <table border="1" cellPadding={editMode ? "0" : "1"}><tr>
          {columns
